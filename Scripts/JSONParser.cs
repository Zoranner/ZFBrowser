...
namespace ZenFulcrum.EmbeddedBrowser
{
    ...
    internal static class JSONParser
    {
        ...
        {
            var table = new JSONNode(JSONNode.NodeType.Object);
            int token;
            // {
        ...
        {
            var array = new JSONNode(JSONNode.NodeType.Array);
            // [
            NextToken(json, ref index);
            var done = false;
            while (!done)
                    ...
                    else if (c == 'u')
                    {
                        ...
                        if (remainingLength >= 4)
                        {
                            ...
                            if (0xD800 <= codePoint && codePoint <= 0xDBFF) // if high surrogate
                            {
                                ...
                                {
                                    if (new string(json, index, 2) == "\\u" && uint.TryParse(new string(json, index + 2, 4), NumberStyles.HexNumber, CultureInfo.InvariantCulture, out var lowCodePoint))
                                    {
