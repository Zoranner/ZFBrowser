...
namespace ZenFulcrum.EmbeddedBrowser
{
     ...
     */
    internal class PostBuildStandalone
    {
        ...
        public static void PostprocessLinuxOrWindowsBuild(BuildTarget target, string buildFile)
        {
            ...
            if (linux)
            {
                MakeExecutable(pluginsPath + FileLocations.SlaveExecutable + exeExt);
            }
		...
		if (target != BuildTarget.StandaloneOSXIntel64) return;
            ...
            if (!Directory.Exists(buildPath + "/Contents/Plugins"))
            {
        ...
        [DllImport("__Internal")] private static extern int symlink(string destStr, string symFile);
        [DllImport("__Internal")] private static extern int chmod(string file, int mode);
