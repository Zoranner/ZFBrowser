...
namespace ZenFulcrum.EmbeddedBrowser
{
    ...
    public class BrowserEditor : Editor
    {
        ...
        private static readonly BrowserNative.FrameCommand[] commandValues;
        ...
        private void RenderActions()
        {
            ...
            {
                if (GUILayout.Button("Go to URL"))
                {
                    browser.LoadURL(serializedObject.FindProperty("_url").stringValue, false);
                }

                if (GUILayout.Button("Force to URL"))
                {
                    browser.Url = serializedObject.FindProperty("_url").stringValue;
                }
                ...
                {
                    browser.Zoom = serializedObject.FindProperty("_zoom").floatValue;
                }
            }
            ...
            GUILayout.BeginHorizontal();
            {
                if (GUILayout.Button("Show Dev Tools"))
                {
                    browser.ShowDevTools();
                }

                if (GUILayout.Button("Hide Dev Tools"))
                {
                    browser.ShowDevTools(false);
                }
            }
            GUILayout.EndHorizontal();
            ...
            var pVal = EditorGUILayout.Popup("Send Command:", -1, commandNames);
            if (pVal != -1)
            {
